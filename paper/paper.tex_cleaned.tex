...
我们定义一条在理想物理约束下的单体推球基准轨迹为 $\tau_{base}$。对于当前协同演进中的随机状态-动作对 $(s, a)$，其原始奖励信号为 $R(s, a)$。

\begin{mathBox}{Advantage 函数重构}
引入控制变量 $B(s)$（即基准轨迹的价值估计），构建重构后的优势函数 $\hat{A}$：
\begin{equation}
...
\end{equation}
其中，$\eta$ 为自适应学习率，$\pi_{push}$ 为当前策略。
\end{mathBox}

该加权机制确保了系统在非平稳的协同环境下，依然能够精准捕捉到超越“平庸基准”的有用信号，有效防止了由于碰撞噪声导致的梯度爆炸。
